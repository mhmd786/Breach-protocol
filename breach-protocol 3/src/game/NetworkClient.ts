...
export class NetworkClient {
  ...
  // BUG FIX ("players float in air" / "can't see each other"): the local player broadcasts
  // player.pos.y, which the engine defines as EYE height (floorY + PLAYER_H, see
  // BreachProtocolEngine.PLAYER_H). Every consumer of a remote player's position, though —
  // the humanoid model (its root group's origin is its FEET, same as bots), the wallhack/
  // intel-ping code, and raycasts against rp.mesh — treats that position as floor height,
  // exactly like bot.mesh.position. Storing the raw eye-height value in rp.pos/targetPos
  // therefore rendered every remote player's model ~1.7m above the ground: floating, shoved
  // up into ceilings/upper floors and out of normal sightlines, with gunfire raycasting
  // against that wrongly-placed mesh instead of where the player visually appears to stand.
  // Converting once here, at the network boundary, keeps rp.pos meaning floor height
  // everywhere downstream, consistent with how bots already work.
  private eyeToFloorY(eyeY: number): number {
    const playerH = this.engine?.PLAYER_H ?? 1.7;
    return eyeY - playerH;
  }
  ...
  private handleMessage(msg: any) {
    switch (msg.type) {
      ...
      case 'player_moved': {
        ...
        if (rp) {
          if (msg.pos) rp.targetPos.set(msg.pos.x, this.eyeToFloorY(msg.pos.y), msg.pos.z);
  ...
  private addOrUpdateRemotePlayer(data: any) {
    ...
    if (!rp) {
      // Default (no pos yet from server) is floor height 0, matching bot spawn convention;
      // a real incoming eye-height value is converted down to floor height via eyeToFloorY.
      const floorY = data.pos?.y !== undefined ? this.eyeToFloorY(data.pos.y) : 0;
      const pos = new THREE.Vector3(data.pos?.x || 0, floorY, data.pos?.z || 0);
      rp = {
        id: data.id,
      ...
      if (data.pos) rp.targetPos.set(data.pos.x, this.eyeToFloorY(data.pos.y), data.pos.z);
