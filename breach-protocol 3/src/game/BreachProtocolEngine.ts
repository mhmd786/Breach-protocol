...
export class BreachProtocolEngine {
  ...
  // Tab equip/holster toggle: press Tab once to raise Montagne's/Blackbeard's shield gadget,
  // press it again to put it away and return to the weapon. No need to hold the key down.
  // Reuses the same this.player.shielded flag damageUnit()/damagePlayer() already check.
  public toggleHeldGadget() {
    ...
    if (op.id !== 'montagne' && op.id !== 'blackbeard') return;
  ...
  // equipped, make sure it doesn't stay stuck on for an operator that no longer has one.
