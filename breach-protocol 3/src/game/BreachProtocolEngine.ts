...
export class BreachProtocolEngine {
  ...
  public initFirstPersonRig() {
    ...
    // actually holding a gadget (currently: [G]-hold to place a Valkyrie/Maestro
    // camera). Swapping visibility with the weapon rig is cheaper than rebuilding
    // either rig every time the gadget is picked up or put away.
  ...
  // Swap the first-person view between the weapon rig and the hand-held gadget rig.
  // Called whenever isHoldingThrow changes so the viewmodel always matches what the
  // player is actually about to do with [G].
  public updateGadgetViewmodel() {
    if (!this.gadgetViewmodelGroup) return;
    const holding = this.isHoldingThrow;
  ...
  private onMouseDown(e: MouseEvent) {
    ...
    if (this.inDroneMode) {
      ...
      // fires a taser dart instead of doing nothing while piloting.
      if (e.button === 0 && this.drone && this.drone.canFireShock()) {
        this.fireDroneShock();
      }
      return;
    }
  ...
  private onKeyDown(e: KeyboardEvent) {
    ...
    // Tab is used to hold up shield gadgets (Montagne/Blackbeard) — prevent the browser's
    // default focus-cycling behavior from stealing keyboard focus off the game canvas.
    if (e.code === 'Tab') {
      e.preventDefault();
    }
  ...
  public handleBreachKey() {
    ...
    if (hits.length > 0) {
      ...
      this.log('Aim at a barricade or wall to plant Breach Charge [B]');
    }
  }
  ...
  public updateMovement(dt: number) {
    ...
    const baseSpeed = 4.2 * (this.player.speedBoost || 1) * (this.player.crouching ? 0.55 : 1);
  ...
  // Twitch's Shock Drone fire: a short-range hitscan taser dart from the drone's own camera,
  // reusing damageUnit()/stun the same way Blitz/Ying's concussion effects do, rather than
  // inventing a second damage pipeline.
  public fireDroneShock() {
    if (!this.drone) return;
    this.drone.shockCooldown = 1.1;
    this.sound.playShockZap();
    ...
    const rc = new THREE.Raycaster(eyePos, dir, 0.05, 14);
    const enemySide = this.player.side === 'atk' ? 'def' : 'atk';
    const targets = this.bots.filter(b => b.alive && b.side === enemySide);
    let hitBot: BotUnit | null = null;
    let bestD = Infinity;
    for (const b of targets) {
      const toBot = b.mesh.position.clone().sub(eyePos);
      const d = toBot.length();
      if (d > 14) continue;
      const angle = toBot.normalize().angleTo(dir);
      if (angle < 0.09 && d < bestD) {
        // Confirm nothing solid is in the way before crediting the hit.
        const wallHits = rc.intersectObjects(this.colliderMeshes());
        if (wallHits.length > 0 && wallHits[0].distance < d - 0.3) continue;
        bestD = d;
        hitBot = b;
      }
    }
    if (hitBot) {
      this.damageUnit(hitBot, 15);
      hitBot.stunned = Math.max(hitBot.stunned, 1.0);
      this.log(`[TWITCH] Shock Drone taser dart shocked ${hitBot.op.name}!`);
    } else {
      this.log('[TWITCH] Shock Drone taser dart discharged!');
    }
  }
  ...
  public updateTrajectoryPreview() {
    const type = this.getThrowableTypeForOperator();
    if (!this.isHoldingThrow || !type || !this.scene) {
  ...
  // Hold-to-use gadgets: Tab raises/lowers Montagne's/Blackbeard's shield for exactly as
  // long as it's held, instead of the old click-to-toggle-forever behavior. Reusing the
  // same this.player.shielded flag damageUnit()/damagePlayer() already check.
  public updateHeldGadgets() {
    ...
    if (op.id === 'montagne' || op.id === 'blackbeard') {
      const holding = !!this.keys['Tab'];
      if (holding !== this.player.shielded) {
        this.player.shielded = holding;
        if (op.id === 'montagne') this.player.isFullShieldExtended = holding;
        this.sound.playGadgetDeploy();
        this.log(holding
      ...
      // Safety net: if the operator changed (side swap/loadout change) while a shield was
      // held up, make sure it doesn't stay stuck on for an operator that no longer has one.
      this.player.shielded = false;
      this.player.isFullShieldExtended = false;
    }
  }
  ...
  public useAbility() {
    ...
    switch (op.id) {
      ...
      case 'twitch': {
        // BUG FIX: previously just an instant raycast zap with no drone at all. Twitch's
        // Shock Drone is meant to be piloted like the recon drone — reuses that exact
        // same ReconDrone class/camera-control system (this.drone/inDroneMode), just
        // spawned from a charge instead of the free prep-phase reserve, and flagged
        // armed so left-click fires a taser dart (see fireDroneShock()) instead of
        // doing nothing while piloting it.
        if (this.inDroneMode) {
      ...
      case 'montagne': {
        // Shield raise/lower is now hold-based on [TAB] (see updateHeldGadgets()) instead of a
        // toggle on the ability key, so holding the shield up doesn't cost a discrete "use".
        this.log('[MONTAGNE] Hold [TAB] to raise Le Roc shield.');
        break;
      }
      case 'glaz': {
      ...
      case 'blackbeard': {
        // Same hold-based change as Montagne above — see updateHeldGadgets().
        this.log('[BLACKBEARD] Hold [TAB] to raise the TARS rifle shield.');
        break;
      }
      case 'capitao': {
      ...
      case 'valkyrie': {
        this.throwGrenade('valkyrie_cam');
        this.player.gadgetCharges = (this.player.gadgetCharges || 1) - 1;
        this.player.abilityCooldown = 3.0;
        break;
      }
      ...
      case 'mira': {
        // BUG FIX: no-op. Reuses the EXISTING working camera architecture (throwGrenade
        // 'valkyrie_cam' → sticks to the surface hit → joins this.securityCameras →
        // viewable/controllable via CCTV mode) instead of a second, fake "observation
        // window" system. Same mechanism Valkyrie already uses, per the project's own
        // architecture.
        this.throwGrenade('valkyrie_cam');
        this.player.gadgetCharges = (this.player.gadgetCharges || 1) - 1;
        this.player.abilityCooldown = 5.0;
        this.log('[MIRA] Black Mirror mounted — view and control it from CCTV mode [6].');
        break;
      }
      case 'lesion': {
        this.sound.playGadgetDeploy();
      ...
      case 'maestro': {
        // BUG FIX: this was deploying the 'evil_eye' gadget type, which is a decorative
        // mesh only — no way to view through it or fire it. Maestro's actual working
        // turret is the maestro_cam path: throwGrenade('maestro_cam') sticks it to a
        // surface, it joins this.securityCameras with isMaestro:true, the player views
        // and aims it through CCTV mode ([6], toggleCctvMode/nextCctvCamera), and
        // left-click calls fireMaestroLaser() — the exact same mechanism Valkyrie's cam
        // uses (see the 'valkyrie' case below) just with isMaestro's armored HP/laser.
        this.throwGrenade('maestro_cam');
        this.player.gadgetCharges = (this.player.gadgetCharges || 1) - 1;
        this.player.abilityCooldown = 5.0;
        break;
      }
