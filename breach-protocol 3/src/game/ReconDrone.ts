...
export class ReconDrone {
  ...
  // `isArmed` flags a drone as hers (drawn with a mounted micro-gun barrel, and lets the
  // pilot actually fire live rounds from it while piloting).
   ...
   */
  // Marks this drone as Twitch's Shock Drone: adds a visible mounted micro-gun barrel so it
  // reads differently from a plain recon drone, and lets update()/canFire() know live-fire
  // is allowed while piloting it.
  public setArmed() {
    this.isArmed = true;
    const barrelMat = new THREE.MeshStandardMaterial({ color: 0x2a2e33, roughness: 0.35, metalness: 0.85, emissive: 0x1a0800, emissiveIntensity: 0.25 });
    const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.014, 0.11, 8), barrelMat);
    barrel.rotation.x = Math.PI / 2;
    barrel.position.set(0, 0.065, -0.2);
    this.mesh.add(barrel);
  }

  public canFire(): boolean {
    return this.isArmed && !this.destroyed && this.fireCooldown <= 0;
  }
