...
export default function App() {
  ...
  // Keyboard listeners: O (Scoreboard), T (Chat)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 't' && !chatOpen && inMatch) {
        e.preventDefault();
        setChatOpen(true);
        document.exitPointerLock?.();
      }
  ...
  }, [chatOpen, inMatch, hudState.inCctvMode, showPauseMenu]);
  ...
  // Flashbang whiteout overlay: mutates the div's style directly every frame instead of
  // going through setState, since a fading screen flash needs to update far more often
  // than the rest of the HUD (which only refreshes on discrete game events).
  useEffect(() => {
    if (!inMatch) return;
    let raf: number;
    const tick = () => {
      ...
      if (eng && el) {
        const v = Math.max(0, Math.min(1, eng.flashWhiteout));
  ...
  return (
    <div id="app" className="relative w-screen h-screen overflow-hidden bg-black text-[#e8f0ff] select-none font-sans">
      ...
      {screenState === 'main_menu' && (
        <div id="menu" className="absolute inset-0 z-20 flex flex-col items-center justify-start gap-4 bg-[radial-gradient(ellipse_at_center,#0d1620_0%,#030508_100%)] p-4 overflow-y-auto">
          {/* Authentic R6 Siege Top Navigation Header */}
          <div className="w-full max-w-5xl flex flex-wrap items-center justify-between gap-3 bg-[#0a1118]/95 border-b border-[#2c5771]/60 px-6 py-3 shadow-2xl backdrop-blur-md rounded-xl">
            <div className="flex items-center gap-4">
              <div className="flex flex-col">
                <span className="text-[10px] tracking-[4px] text-gray-400 font-bold uppercase">TOM CLANCY'S</span>
                <span className="text-xl md:text-2xl font-black tracking-widest text-[#ffffff] drop-shadow-[0_0_10px_rgba(255,255,255,0.4)]">
                  RAINBOW<span className="text-[#ff6b4a]">6</span> SIEGE
                </span>
              </div>
              <div className="hidden md:flex items-center gap-1 ml-6 text-xs font-bold tracking-wider">
                <button className="px-4 py-1.5 bg-[#1b2a38] text-cyan-300 border-b-2 border-cyan-400 rounded-t">HOME</button>
                <button onClick={() => setShowSkinShop(true)} className="px-4 py-1.5 text-gray-300 hover:text-white hover:bg-[#13202b] transition-all rounded-t">OPERATORS</button>
                <button onClick={() => setShowSkinShop(true)} className="px-4 py-1.5 text-gray-300 hover:text-white hover:bg-[#13202b] transition-all rounded-t">SHOP</button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap text-xs font-mono">
              ...
              <div className="px-3 py-1 bg-amber-950/60 border border-amber-500/50 rounded-lg text-amber-300 font-bold flex items-center gap-1.5 shadow-inner">
                <span>💰</span>
                <span>{rankedProfile.renown.toLocaleString()}</span>
              </div>
              ...
              <button
                ...
                className="px-3 py-1 bg-blue-900/60 hover:bg-blue-800 text-cyan-200 border border-blue-500/50 rounded-lg font-bold flex items-center gap-1.5 cursor-pointer transition-all shadow-md active:scale-95"
          ...
          <div className="w-full max-w-5xl flex items-center justify-between px-2">
            <div>
              <h1 className="text-2xl md:text-3xl tracking-[4px] text-[#7fd6ff] font-black uppercase">
                SQUAD OPERATIONS
              </h1>
              <p className="text-xs text-[#a0c4db] tracking-widest">Active Tactical Squad · 5v5 Competitive Ranked & Custom LAN Lobbies</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="px-3 py-1 bg-[#132433] border border-cyan-500/40 rounded text-xs font-mono text-cyan-300">
                STATUS: ONLINE (12ms)
              </span>
            </div>
          </div>

          <div className="bg-[#0f1923]/95 border border-[#7fd6ff]/30 rounded-xl p-5 w-full max-w-5xl shadow-2xl backdrop-blur-md flex flex-col gap-4">
      ...
      {inMatch && (
        <div id="hud" className="absolute inset-0 pointer-events-none z-10">
          {/* Flashbang whiteout — opacity driven directly by engine.flashWhiteout every frame */}
          <div
            ref={flashOverlayRef}
