...
export class BreachProtocolEngine {
  ...
  public sound = {
    ...
    breachExplode: () => {
      ...
      setTimeout(() => this.sfx(120, 0.4, 'square', 0.25), 30);
    },
    rappelHook: () => this.sfx(700, 0.15, 'triangle', 0.09),
    woodSnap: () => this.sfx(150, 0.1, 'square', 0.1),
    pingHostile: () => {
      this.sfx(880, 0.1, 'sine', 0.12);
      setTimeout(() => this.sfx(1100, 0.12, 'sine', 0.14), 80);
    },
    pingObjective: () => {
      this.sfx(600, 0.12, 'triangle', 0.12);
      setTimeout(() => this.sfx(900, 0.15, 'triangle', 0.14), 100);
      setTimeout(() => this.sfx(1200, 0.2, 'triangle', 0.15), 220);
    },
  ...
  public startMatch(config: MatchConfig) {
    ...
    const mapNames: Record<string, string> = {
      ...
      harbor_villa: 'Harbor Safehouse',
      chalet: 'Alpine Chalet',
  ...
  public rebuildHouseMap() {
    ...
    if (this.currentMapKey === 'harbor_villa') {
      skyColor = 0x79c5e6;
      fogDensity = 0.01;
      hemiSky = 0xd0f4ff;
      hemiGround = 0x24343a;
      ...
      fillColor = 0x408099;
      fillIntensity = 0.55;
    } else if (this.currentMapKey === 'chalet') {
      skyColor = 0x0f1524;
      fogDensity = 0.02;
      hemiSky = 0x3a4b6e;
      hemiGround = 0x080a12;
      ...
      fillColor = 0x1f283d;
      fillIntensity = 0.2;
    } else if (this.currentMapKey === 'warehouse') {
  ...
  public spawnBreachExplosion(pos: THREE.Vector3, normal: THREE.Vector3) {
    ...
    setTimeout(() => this.scene.remove(fireSphere), 200);
  }
  ...
  public getFloorHeight(x: number, z: number, currentFeetY: number = 0): number {
    ...
    // Dedicated two-storey layouts use their own footprints and stair positions.
    if (this.currentMapKey === 'harbor_villa') {
      if (x >= -2 && x <= 0.5 && z >= -5 && z <= 2.5) return Math.max(0.02, Math.min(3.6, (z + 5) / 7.5 * 3.6));
      return currentFeetY > 2 && x >= -16 && x <= 16 && z >= -12 && z <= 12 ? 3.6 : 0.02;
    }
    if (this.currentMapKey === 'chalet') {
      if (x >= 6 && x <= 8.5 && z >= -4 && z <= 4) return Math.max(0.02, Math.min(3.8, (z + 4) / 8 * 3.8));
      return currentFeetY > 2 && x >= -18 && x <= 18 && z >= -14 && z <= 14 ? 3.8 : 0.02;
    }
  ...
  public useAbility() {
    ...
    switch (op.id) {
      ...
      case 'twitch': {
        this.sound.playShockZap();
        const hitTarget = this.bots.find(b => b.alive && b.side !== this.player.side && b.mesh.position.distanceTo(eyePos) < 15);
        if (hitTarget) {
          this.damageUnit(hitTarget, 15);
          this.log(`[TWITCH] Shock Drone taser dart shocked ${hitTarget.op.name}!`);
        } else {
      ...
      case 'montagne': {
        ...
        this.player.isFullShieldExtended = this.player.shielded;
        this.sound.playGadgetDeploy();
        this.player.abilityCooldown = 1.0;
        this.log(this.player.shielded ? '[MONTAGNE] Le Roc Extended — 100% Frontal Bullet Immunity!' : '[MONTAGNE] Le Roc Retracted.');
        break;
      }
      case 'glaz': {
      ...
      case 'blackbeard': {
        ...
        this.sound.playGadgetDeploy();
        this.player.abilityCooldown = 1.0;
        this.log(this.player.shielded ? '[BLACKBEARD] TARS Rifle Shield mounted!' : '[BLACKBEARD] Rifle Shield removed.');
        break;
      }
      case 'capitao': {
      ...
      case 'dokkaebi': {
        ...
        setTimeout(() => { this.player.isDokkaebiRinging = false; }, 8000);
        this.player.gadgetCharges = (this.player.gadgetCharges || 1) - 1;
      ...
      case 'nokk': {
        ...
        setTimeout(() => { this.player.isSilencedFootsteps = false; }, 12000);
      ...
      case 'solid_snake': {
        ...
        setTimeout(() => { this.player.cloaked = false; }, 10000);
        this.player.abilityCooldown = 15.0;
      ...
      case 'caveira': {
        this.player.isSilencedFootsteps = true;
        setTimeout(() => { this.player.isSilencedFootsteps = false; }, 10000);
        this.player.abilityCooldown = 12.0;
      ...
      case 'vigil': {
        this.sound.playPhoneBuzz();
        this.player.cloaked = true;
        setTimeout(() => { this.player.cloaked = false; }, 12000);
        this.player.abilityCooldown = 15.0;
  ...
  public triggerBotGadget(bot: BotUnit) {
    ...
    switch (op.id) {
      ...
      case 'montagne':
        bot.shielded = true;
        setTimeout(() => { bot.shielded = false; }, 15000);
        this.log(`[ATK-${op.name}] Le Roc shield extended!`);
        break;
      case 'blackbeard':
        bot.shielded = true;
        setTimeout(() => { bot.shielded = false; }, 8000);
      ...
      case 'vigil':
        bot.cloaked = true;
        setTimeout(() => { bot.cloaked = false; }, 12000);
        this.log(`[DEF-${op.name}] ERC-7 electronic cloak active!`);
        break;
      case 'nokk':
        bot.cloaked = true;
        setTimeout(() => { bot.cloaked = false; }, 10000);
      ...
      case 'amaru':
        bot.speedBoost = 1.8;
        setTimeout(() => { bot.speedBoost = 1; }, 2000);
        this.log(`[ATK-${op.name}] Garra Hook grapple burst!`);
        break;
      case 'caveira':
        bot.speedBoost = 1.3;
        setTimeout(() => { bot.speedBoost = 1; }, 6000);
